...
def train(device, model, disc, train_data_loader, test_data_loader, optimizer, disc_optimizer,
    ...
    while global_epoch < nepochs:
        ...
        writer.add_scalar("Percep_Loss/train", running_perceptual_loss/len(train_data_loader), global_epoch)
        writer.add_scalar("Fake_loss/train", running_disc_fake_loss/len(train_data_loader), global_epoch)
        writer.add_scalar("Real_loss/train", running_disc_real_loss/len(train_data_loader), global_epoch)
...
def eval_model(test_data_loader, global_step, device, model, disc):
    ...
    while 1:
        ...
        writer.add_scalar("L1_Loss/val", sum(running_l1_loss) / len(running_l1_loss), global_step)
        writer.add_scalar("Sync_Loss/val", sum(running_sync_loss) / len(running_sync_loss), global_step)
        writer.add_scalar("Percep_Loss/val", sum(running_perceptual_loss) / len(running_perceptual_loss), global_step)
        writer.add_scalar("Fake_loss/val", sum(running_disc_fake_loss) / len(running_disc_fake_loss), global_step)
        writer.add_scalar("Real_loss/val", sum(running_disc_real_loss) / len(running_disc_real_loss), global_step)
